...
void conditionCheck(struct Expr *expr, const char *conditionType) 
{
  ...
  else if (expr->type->type != Type_BOOL) semanticError("operand of %s statement is not boolean type\n", conditionType);
}
...
void assignCheck(struct Expr *var, struct Expr *expr) 
{
    ...
    if(var->op == Op_VAR)
    {
        ...
        //Constant Variable
        if(a->constant) semanticError("constant Variable '%s' cannot be assigned\n", var->name);
    }
}
